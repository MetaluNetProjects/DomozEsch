...
int16_t accel[3];
int16_t quat[4];*/
#define vBat *((uint8_t*)(outBuf + 1) + 0)
//#define accel ((int16_t*)(outBuf + 1) + 1)
//#define quat ((int16_t*)(outBuf + 1) + 4)

#define analogs ((uint8_t*)(outBuf + 1) + 1)

void aquireChan(int chan, int pin)
{
   //digitalWrite(pin, true);
   pinMode(pin, INPUT_PULLUP);
   pinMode(pin, INPUT);
   //analogs[chan] = analogRead(pin) >> 2;
   analogs[chan] = digitalRead(pin) * 255;
}

void aquire() {
  //uint8_t buffer[8];
  vBat = analogRead(VBATPIN) >> 2;
...
//  bno.readLen(bno.BNO055_QUATERNION_DATA_W_LSB_ADDR, (char*)quat, 8);
  /*analogs[0] = analogRead(A0) >> 2;
  analogs[1] = analogRead(A1) >> 2;
  analogs[2] = analogRead(A2) >> 2;
  analogs[3] = analogRead(A3) >> 2;
  analogs[4] = analogRead(A4) >> 2;
  analogs[5] = analogRead(A5) >> 2;
  analogs[6] = analogRead(A7) >> 2;
  analogs[7] = analogRead(A10) >> 2;
  analogs[8] = analogRead(A11) >> 2;*/
