...
void aquireVbatt() {
  outBuf.vbatt = analogRead(VBATPIN) >> 2;
}
...
void readCommand()
{
  ...
  if(Serial.available()) {
    ...
    if(c == 10) {
      ...
      else if(commandLen >= 1 && command[0] == 'B') {
        Serial.print("bno temp: "); Serial.println(bno.getTemp());
      }
      commandLen = 0;
    } else {
...
void rfService()
{
  ...
  if (rf69.available()) {
    ...
    if (rf69.recv(inBuf, &len)) {
      ...
      // Send a reply!
      rf69.send((uint8_t*)&outBuf, sizeof(t_outbuf));
...
{
    bno.readLen(bno.BNO055_QUATERNION_DATA_W_LSB_ADDR, dest, 8);
}

void bnoGetAcc(void* dest) // 3 ints = 6 chars
{
    bno.readLen(bno.BNO055_LINEAR_ACCEL_DATA_X_LSB_ADDR, dest, 6);
}
